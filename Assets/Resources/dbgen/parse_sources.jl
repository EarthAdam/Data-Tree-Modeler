...
const EXTENSIONS = [".c", ".cxx", ".cpp"]   # TODO: should also parse headers
...
function main(args)
    ...
    # HACK: get all valid function names, for easy matching in parse_callgrind.jl
    file_fns = Dict{String,Vector{String}}()
    function get_fns!(node)
        for (id, child) in node.nodes
            if isa(child, Node{FileNode})
                file_fns[id] = [k for (k,v) in child.nodes if isa(v, Node{FunctionNode})]
            else
                get_fns!(child)
            end
        end
    ...
    get_fns!(tree)
    open("functions.dat", "w") do io
        serialize(io, file_fns)
    end
end
main(ARGS)
